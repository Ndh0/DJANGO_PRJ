    ...
    if request.method == 'POST' :
        ...
        if form.is_valid():
            ...
            if hasattr(user, 'client'):
                return redirect('addclient', id=user.client.id)
...
def adminperslist(request):
    list=CustomUser.objects.filter(user_type='personnel')
    return render( request,'admin/adminperslist.html',{'list':list}) 

def admincltlist(request):
    list=CustomUser.objects.filter(user_type='client')
    return render( request,'admin/admincltlist.html',{'list':list}) 

def adminprjlist(request):   
    list=Projet.objects.filter(status=True)
    return render( request,'admin/adminprjlist.html',{'list':list})
...
def admin_panel(request):
    ...
    return render(request, 'admin/projetAa.html', {'projects_to_approve': projects_to_approve})
...
@login_required
def ClientAdd(request, id):
    client = get_object_or_404(Client, id=id, user=request.user)

    if request.method == 'GET':
        context = {'form': ClientForm(instance=client), 'id': id}
        return render(request, 'ArtyProd/addclient.html', context)
